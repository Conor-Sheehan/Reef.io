...
class SignInVC: UIViewController, UITextFieldDelegate {
  ...
  func displayAuthFailedError(error: NSError) {
    ...
    case .wrongPassword:
      showInvalidErrorMessage(textField: passwordTextField,
                              invalidLabel: passwordInvalidLabel,
                              message: "Incorrect password. Please try again")
      return
    case .invalidEmail:
      showInvalidErrorMessage(textField: emailTextField,
                              invalidLabel: emailInvalidLabel,
...
extension SignInVC {
  ...
  /// CALLED WHEN KEYBOARD IS PROMPTED TO APPEAR ON THE SCREEN
   ...
   func textFieldShouldReturn(_ textField: UITextField) -> Bool {
     ...
     if textField == emailTextField { passwordTextField.becomeFirstResponder() } else {
      // Attempt to Log in
      signIn(self)
      self.view.endEditing(true)
    }
  ...
  /// GESTURE RECOGNIZES USER SWIPE DOWN TO DISMISS KEYBOARD
    ...
    func addKeyboardObservers() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(SignInVC.keyboardWillShow),
                                               name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(SignInVC.keyboardWillHide),
  ...
  func textFieldDidEndEditing(_ textField: UITextField) {
     ...
     case emailTextField:
       guard let email = emailTextField.text else { return }
       if !isValidEmail(email) { showInvalidErrorMessage(textField: emailTextField,
                                                         invalidLabel: emailInvalidLabel,
