...
class SignUpVC: UIViewController, UITextFieldDelegate {
  ...
  func checkTextFields(email: String, password: String, confirmPassword: String) -> Bool {
    ...
    if email == "" {
      ...
      showInvalidErrorMessage(textField: passwordTextField, invalidLabel: passwordInvalidLabel, message: "Please enter a valid password")
      return false
    }
    else if confirmPassword == "" {
      showInvalidErrorMessage(textField: confirmPasswordTextField, invalidLabel: confirmInvalidLabel, message: "Please confirm your password")
      return false
    }
    
    // Verify that the password and confirmed password are the same strings
    else if password != confirmPassword {
      showInvalidErrorMessage(textField: confirmPasswordTextField, invalidLabel: confirmInvalidLabel, message: "Passwords don't match. Try again.")
      return false
    }
  ...
  func displayAuthFailedError(error: NSError) {
    ...
    case .emailAlreadyInUse:
     ...
     showInvalidErrorMessage(textField: emailTextField, invalidLabel: emailInvalidLabel, message: "Email already taken. Try again.")
      return
    case .invalidEmail:
     emailTextField.becomeFirstResponder()
     showInvalidErrorMessage(textField: emailTextField, invalidLabel: emailInvalidLabel, message: "Please enter a valid email address.")
     return
    case .weakPassword:
     self.passwordTextField.becomeFirstResponder()
     showInvalidErrorMessage(textField: passwordTextField, invalidLabel: passwordInvalidLabel, message: "Invalid password. Must be at least 6 characters")
     return
    default:
...
extension SignUpVC {
  ...
  func textFieldDidEndEditing(_ textField: UITextField) {
    ...
    case passwordTextField:
      guard let password = passwordTextField.text else { return }
      if password.count < 6 { showInvalidErrorMessage(textField: passwordTextField, invalidLabel: passwordInvalidLabel, message: "Invalid password. Must be at least 6 characters") }
    default:
      guard let password = passwordTextField.text else { return }
      guard let confirmPassword = confirmPasswordTextField.text else { return }
