...
extension AppBrain {
    ...
    func readGrowData() {
        ...
        if let firebaseUID = userUID {
          ...
          let growDataRef = databaseRef?.child("Users").child(firebaseUID).child("GrowData")

          growDataRef?.child("CurrentPlantHeight").observeSingleEvent(of: .value) { (snapshot) in
                if let currentPlantHeight = snapshot.value as? Int {
                    self.currentPlantHeight = currentPlantHeight
                }
            }

          growDataRef?.child("SeedlingStartDates").observeSingleEvent(of: .value) { (snapshot) in
